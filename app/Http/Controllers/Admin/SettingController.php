...
class SettingController extends Controller
{
    ...
    public function saveSystemSettings(Request $request)
    {
        ...
        try {
            ...
            System::updateOrCreate(
                ['key' => 'active_filter_category'],
                ['value' => !empty($request->active_filter_category) ? 1 : 0, 'date_and_time' => Carbon::now(), 'created_by' => Auth::user()->id]
            );
            System::updateOrCreate(
                ['key' => 'active_image_product'],
                ['value' => !empty($request->active_image_product) ? 1 : 0, 'date_and_time' => Carbon::now(), 'created_by' => Auth::user()->id]
            );
            System::updateOrCreate(
                ['key' => 'active_image_category'],
                ['value' => !empty($request->active_image_category) ? 1 : 0, 'date_and_time' => Carbon::now(), 'created_by' => Auth::user()->id]
            );
            System::updateOrCreate(
                ['key' => 'active_underline'],
                ['value' => !empty($request->active_underline) ? 1 : 0, 'date_and_time' => Carbon::now(), 'created_by' => Auth::user()->id]
            );
